...
-- Note: incremental search is on by default
...
vim.keymap.set('n', '<leader>d', vim.diagnostic.open_float, { desc = 'Show [D]iagnostic Error messages' }) -- <leader>e is already used for explorer
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sg', builtin.grep_string, { desc = '[S]earch by [G]rep (non-fuzzy)' })
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      -- Disable format on save for now - not setting it means disable
      -- format_on_save = function(bufnr)
      --   -- Disable "format_on_save lsp_fallback" for languages that on't
  ...
  { -- Highlight, edit, and navigate code
    ...
    build = ':TSUpdate',
    opts = {
      ensure_installed = { 'bash', 'c', 'html', 'lua', 'luadoc', 'markdown', 'vim', 'vimdoc', 'c_sharp', 'typescript' },
...
vim.keymap.set({ 'n', 'v' }, '<C-m>', '<C-a>', { remap = true })
...
vim.cmd [[highlight MatchParen gui=underline guibg=transparent guifg=orange]]
